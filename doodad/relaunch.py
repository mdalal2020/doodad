...
def checkpoint_cmd(docker_name, chk_name, chk_dir="/docker_checkpoints"):
    return "docker checkpoint create --checkpoint-dir=%s %s %s" % (
        chk_dir,
        docker_name,
        chk_name,
    )
...
def checkpoint_restore_cmd(docker_name, checkpoint_name, chk_dir="/docker_checkpoints"):
    return "docker start --checkpoint-dir=%s --checkpoint=%s %s" % (
        chk_dir,
        checkpoint_name,
        docker_name,
    )
...
class CheckpointManager(object):
    ...
    def checkpoint_and_tar_cmd(self, docker_name, tar_name, restart=True):
        ...
        chk_cmd = checkpoint_cmd(
            docker_name, checkpoint_name, chk_dir=self.checkpoint_dir
        )
        cmds.append(chk_cmd)
        ...
        if restart:
            restart_cmd = checkpoint_restore_cmd(
                docker_name, checkpoint_name, chk_dir=self.checkpoint_dir
            )
    ...
    def checkpoint_tar_loop_cmd(self, docker_name, tar_name, wait_interval=1):
        ...
        """.format(
            wait_interval=wait_interval, chk_tar_cmd=chk_tar_cmd
        )
